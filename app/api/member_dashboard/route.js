...
const formatAlertMessage = (title, email, ipAddress, userAgent, additionalInfo = '') => {
  return `PLAY MAKER ARENA MEMBER DASHBOARD\n--------------------------------------------------\n${title}\nEmail : ${email}\nIP : ${ipAddress}\nDevice INFO : ${userAgent}${additionalInfo}`;
};
...
export async function GET(request) {
  ...
  try {
    ...
    const result = await query(
      `SELECT pma_id, first_name, last_name, email, profile_image, type, 
       status, created_at, updated_at FROM pma_member_info WHERE email = $1`,
      [email]
    );
