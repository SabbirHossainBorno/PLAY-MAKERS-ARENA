...
const formatAlertMessage = (title, email, ipAddress, additionalInfo = '') => {
  return `PLAY MAKER ARENA AUTH-CHECKER\n----------------------------------------\n${title}\nEmail : ${email}\nIP : ${ipAddress}${additionalInfo}`;
};
...
export async function GET(request) {
  ...
  if (!email || !sessionId) {
    const alertMessage = `PLAY MAKER ARENA AUTH-CHECKER\n----------------------------------------\n🚨 Unauthorized Access Attempt!\nIP : ${ip}\nUA : ${userAgent}`;
  ...
  try {
    ...
    if (email.endsWith('@pma.com')) {
      const adminRes = await query(
        'SELECT type, status FROM pma_admin_info WHERE email = $1',
        [email]
      );
      ...
      const memberRes = await query(
        `SELECT type, status FROM pma_member_info 
