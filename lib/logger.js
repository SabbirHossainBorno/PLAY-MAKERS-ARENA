...
const archivePreviousLogFile = () => {
  ...
  const logFile = path.join(logDir, `play-makers-arena-${yesterday}.log`);
  const archiveFile = path.join(archiveDir, `play-makers-arena-${yesterday}.log`);
...
const logger = createLogger({
  ...
  format: format.combine(
    format.timestamp({
      format: () => moment().format('YYYY-MM-DD HH:mm:ss') // Using local server time
    }),
  ...
  transports: [
    ...
    new transports.DailyRotateFile({
      filename: 'logs/play-makers-arena-%DATE%.log',
